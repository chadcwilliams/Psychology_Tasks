%% Decision Making Task for Muse LSL and VPixx
...
%% Marker List
%101: Fixation for side 1 (where left side is correct answer)
%102: Fixation for side 2 (where right side is correct answer)
%111: Draw squares for side 1 (where left side is correct answer)
%112: Draw squares for side 2 (where right side is correct answer)
%121: Go cue for side 1 (where left side is correct answer)
%122: Go cue for side 2 (where right side is correct answer)
%131: Fixation for side 1 (where left side is correct answer)
%132: Fixation for side 2 (where right side is correct answer)
%141: Win when got correct
%142: Win when got incorrect
%143: Loss when got incorrect
%144: Loss when got correct
%145: Too slow
...
pname = strcat('DecMakBehMuseLSL_',subject_number);
...
for block = 1:num_of_blocks
    ...
    for trial = 1:num_of_trials
        ...
        DrawFormattedText(win,'+','center','center',[75 75 75]);
        if LSL_or_VPixx == 1
            LSL_flipandmark(marker+20,win,outlet,usingMuse); %%%WHY IS IT THE SAME MARKER
        elseif LSL_or_VPixx == 2
            VPixx_flipandmark(win,marker+20,usingVPixx);
        else
            Screen('Flip',win);
        end
        ...
        DrawFormattedText(win,'+','center','center',[255 255 255]);
        if LSL_or_VPixx == 1
            LSL_flipandmark(marker+30,win,outlet,usingMuse);
        elseif LSL_or_VPixx == 2
            VPixx_flipandmark(win,marker+30,usingVPixx);
        else
            Screen('Flip',win);
        end
        ...
        DrawFormattedText(win,Feedback_Msg,'center','center',[255 255 255]);
        if LSL_or_VPixx == 1
            LSL_flipandmark(feedback_marker,win,outlet,usingMuse);
        elseif LSL_or_VPixx == 2
            VPixx_flipandmark(win,feedback_marker,usingVPixx);
        else
            Screen('Flip',win);
        end
